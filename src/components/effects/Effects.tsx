...
export const Spotlight: React.FC<{
  ...
  return <AbsoluteFill style={{ pointerEvents: "none", opacity: motion.opacity, background: `radial-gradient(ellipse ${width / 2}px ${height / 2}px at ${x + width / 2}px ${y + height / 2}px, transparent 0%, transparent 62%, rgba(0,0,0,${dimOpacity}) 100%)` }}>
    <div style={{ position: "absolute", left: x - 8, top: y - 8, width: width + 16, height: height + 16, border: "2px solid #10B981", borderRadius: 14, boxShadow: "0 0 34px rgba(16,185,129,.28)" }} />
  </AbsoluteFill>;
};
...
export const ZoomFocus: React.FC<{ rect: [number, number, number, number]; enter?: ClipAnimation }> = ({ rect, enter }) => {
  ...
  const [x, y, width, height] = rect;
  return <div style={{ position: "absolute", left: x, top: y, width, height, pointerEvents: "none", border: "2px solid rgba(255,255,255,.75)", borderRadius: 12, transform: `scale(${amount})`, boxShadow: "0 0 0 9999px rgba(30,41,59,.18)", opacity: amount }} />;
};
