  ...
  return <div style={{ position: "absolute", left: position[0], top: position[1], width, opacity: motion.opacity, transform: `translate(${motion.x}px, ${motion.y}px) scale(${motion.scale})` }}>
    ...
    <div style={{ display: "flex", alignItems: "center" }}>
      {nodes.map((node, index) => {
        ...
        const color = node.tone ?? (index === nodes.length - 1 ? LIME : CREAM);
        return <React.Fragment key={node.id}>
          ...
          {index < nodes.length - 1 && <div style={{ width: 70, height: 2, background: `linear-gradient(90deg, ${LIME} ${Math.max(0, Math.min(100, (frame - index * 12 - 9) * 7))}%, rgba(16,185,129,.18) 0)`, position: "relative" }}><span style={{ position: "absolute", right: -1, top: -8, color: LIME }}>›</span></div>}
        </React.Fragment>;
      })}
    </div>
  </div>;
