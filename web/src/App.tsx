...
export default function App() {
  ...
  const [outputMode, setOutputMode] = useState<"mp4" | "alpha">("mp4");
  ...
  async function renderProject() {
    ...
    try {
      const renderProjectData = structuredClone(project);
      if (outputMode === "alpha") {
        renderProjectData.format.background = "transparent";
        renderProjectData.scenes.forEach((scene) => {
          scene.background = "transparent";
          scene.layers.forEach((layer) => layer.clips.forEach((clip) => {
            if (clip.kind === "apple.motion" || clip.kind === "apple.iconFootage") clip.props.background = "transparent";
          }));
        });
      }
      const res = await fetch(`${API}/render-project`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project: renderProjectData, audio, name: project.id, outputMode }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Render gagal");
      setVideoUrl(data.url);
      setStatus(outputMode === "alpha" ? "Overlay transparan selesai. Import MOV ke CapCut tanpa Chroma Key." : "Render MP4 selesai.");
    }
    catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
  ...
  return <div className={`motion-app ${advancedVisuals ? "advanced-mode" : "simple-mode"}`}>
    ...
    <main className="motion-main">
      <header className="motion-header"><div><input className="project-title" value={project.title} onChange={(e) => updateProject((p) => ({ ...p, title: e.target.value, id: safeId(e.target.value, p.id) }))} /><span>{project.format.width}×{project.format.height} · {project.format.fps}fps</span></div><div style={{display:"flex",gap:10,alignItems:"center"}}><select value={outputMode} onChange={(e) => setOutputMode(e.target.value as "mp4" | "alpha")} title="Pilih MP4 untuk full-screen atau MOV alpha untuk overlay presenter" style={{height:44,padding:"0 12px",background:"#111827",color:"#fff",border:"1px solid #334155",borderRadius:4}}><option value="mp4">Full-screen MP4</option><option value="alpha">Overlay transparan MOV</option></select><button className="render-button" disabled={loading || !project.scenes.length} onClick={renderProject}>{loading ? "Rendering…" : outputMode === "alpha" ? "▶ Render Alpha" : "▶ Render MP4"}</button></div></header>
